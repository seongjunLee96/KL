							...
							<thead>
								<c:forEach var="pt" items="${ptList}">
									<tr>
										...
										<th>${pt.endtime }</th>
										<th>${pt.count}</th>
										<th><button type="submit" class="byn btn-secondary"
												onclick="location.href='reptmake?log=${pt.log}&id=<%=session.getAttribute("session_id") %>'">수정</button>
											<br></th>
										<th><button type="submit" class="byn btn-secondary"
												onclick="location.href='ptdelete?log=${pt.log}'">삭제</button>
											<br></th>
									</tr>
								</c:forEach>
						</table>
